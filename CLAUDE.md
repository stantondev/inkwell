...
- **Federated Inks (inbound ActivityPub Like → Ink)**: When a Mastodon/fediverse user favorites (Likes) an Inkwell entry, it creates a federated ink record and increments `ink_count`. `Undo { Like }` removes the ink and decrements count. Fediverse inks automatically contribute to trending and discovery. Mutually exclusive FK pattern: `user_id` (local) vs `remote_actor_id` (fediverse) on inks table. Outbound Like for local inking is not implemented (AP has no standard mechanism for origin server to broadcast Like counts).
- **Backend**:
