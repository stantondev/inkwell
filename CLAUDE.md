...
- `POST /api/billing/webhook` → handles Square webhook events (subscription.created, subscription.updated, invoice.payment_made, invoice.payment_failed, dispute.created, payment.completed for one-time donations)
...
- **Admin dashboard**: full admin panel at `/admin` with tabbed navigation (Dashboard / Users / Entries / Reports / Polls). Dashboard shows platform stats (total users, Plus subscribers, signups this week, entries, comments, blocked users, pending reports) plus recent Plus subscribers, recent Ink Donors, and recent signups — each with colored subscription-status badges (Active/Canceled/Past due) and expiration dates on hover. User management at `/admin/users` with search, filtering (All/Admins/Plus/Donors/Active/Inactive/Blocked), admin promotion/demotion, user blocking (revokes all tokens), and account deletion. Tier column shows subscription status inline (Plus + Active/Canceled/Past due badges) and exposes canceled Ink Donor status. Entry management at `/admin/entries` with sensitive toggle per entry. Report management at `/admin/reports` with filter tabs (all/pending/dismissed/actioned) and action buttons (dismiss, mark sensitive, action taken). Admin access via `ADMIN_USERNAMES` env var (bootstrap) or database `role` field (promotable via UI). User schema has `role` (default "user") and `blocked_at` fields. Blocked users are rejected at the auth plug level with `account_blocked` error.
...
- Events: Plus subscription, Ink Donor activation, one-time donations (via `notify_donation/2`, triggered by `payment.completed` webhook), cancellations, payment failures, disputes, new feedback posts
...
### Recently Completed
- **2026-04-15** — Billing Health: Reconcile Rate Limits + Ghost Plus Detection + Sync-by-Email. Followup to 2026-04-08 billing health work. Admin reported the "Reconcile now" button was returning `{:square_error, 429}` for every user and the dashboard was showing `0 Square subs` despite known payers. Three distinct issues fixed; one diagnosed but not fixable from code. **Root cause 1 (reconcile 429 cascade)**: `reconcile_all_users/1` iterated every user sequentially with zero delay, firing 2 Square API calls per user (~160 calls in seconds against Square's ~10 QPS per-merchant limit). `Square.square_post/2` and `square_get/1` had no 429 retry, so the first rate limit cascaded through every subsequent call. **Fix**: extracted the HTTP call from both helpers into a shared private `with_retry/3` function in `apps/api/lib/inkwell/square.ex` (lines 400-425) that sleeps 1000ms on first 429, 2000ms on second, gives up cleanly after 2 retries (max 3 total attempts, 3s worst-case extra latency). Does NOT retry other status codes. Also filtered `reconcile_all_users/1` to only iterate users with billing-relevant state (`subscription_tier = "plus"` OR any donor/Stripe/Square ID set) — shrinks the set from ~80 users to ~5-30, keeps the 300ms inter-user delay under Square's rate ceiling. Reconcile return shape now categorizes errors: `rate_limited` (429 after retries), `not_found` (no Square record OR already-in-sync), `errors` (real errors with up to 20 details). Admin sees "N rate limited (retry), N not found (expected), N real errors" instead of a wall of 429 messages. **Root cause 2 (ghost Plus users invisible)**: `webhook_stats/0` counted `square_subscribers` as users with `square_subscription_id IS NOT NULL AND subscription_status = "active"`, which excluded anyone whose webhook never fired. There was no query for "tier=plus but no payment source of record" — the scariest category. **Fix**: new `Billing.plus_users_by_source/0` (pure DB read, no Square API calls) that categorizes every tier=plus user into 3 buckets — `:square_active` (paying via Square), `:legacy_stripe` (Stripe ID set, dead Stripe account so getting Plus for free), `:orphaned` (tier=plus but NO payment source at all). `webhook_stats/0` extended with `plus_square_active`, `plus_legacy_stripe`, `plus_orphaned` count fields. **New admin endpoint**: `GET /api/admin/plus-users` returns the full categorized list with user details (username, email, signup date, subscription status, all IDs). Renders as collapsible "View Plus user breakdown" section on the admin billing health panel with copy-email buttons per row. No bulk actions — read-only visibility, downgrade decisions are a separate policy call. **New admin endpoint 2**: `POST /api/admin/sync-user-by-email` accepts `{email}`, looks up user (case-insensitive via downcase + trim), calls `sync_from_square/1`, returns categorized result. Single-purpose admin tool for users whose webhook didn't fire — 2 API calls per invocation, no rate limit risk. New `Billing.sync_user_by_email/1` helper. **Root cause 3 (webhook never reaching the API)**: `webhook_deliveries` table empty = route never hit. Not a code bug — Square dashboard misconfiguration (wrong URL, missing subscription, or wrong signature key). **Mitigation**: added a diagnostic banner on the billing health panel showing the expected webhook URL (`https://api.inkwell.social/api/billing/webhook`), highlighted in amber when `last_webhook = never`, so misconfiguration becomes obvious. **UI additions** in `apps/web/src/app/admin/billing-health-panel.tsx`: extended `WebhookStats` and `ReconcileResult` TypeScript types; added `PlusUser` and `PlusUsersData` types; new state hooks for sync form + breakdown open/loading/data/error; webhook URL diagnostic banner above stat cards; 3 new ghost Plus metric cards (On Square green, Legacy Stripe ⚠ amber, Orphaned ⚠ amber); expandable Plus user breakdown with 3 `PlusUserGroup` sections; sync-by-email form with inline result display; categorized reconcile result that shows rate_limited/not_found/errors as separate counts instead of a raw error list. New `HealthStat` `tone` prop for success/warning coloring. **Verified end-to-end**: all 4 admin endpoints exercised against running Phoenix with a real admin token. `billing-health` returns new fields correctly. `plus-users` returns 3 buckets (in local DB copy of production: 0 square_active, 4 legacy_stripe including @zaexpcake @eve @tim @jonhenshaw, 1 orphaned @muse bot). `sync-user-by-email` handles all 4 input variants (valid email, non-existent email, missing email, mixed-case email with whitespace) with correct HTTP status codes. `reconcile-subscriptions` filter shrinks the user set from ~80 to 5, completes in 1.2s locally with proper categorization. All 170 existing Elixir tests still pass. TypeScript check clean. **No migrations, no changes to existing contracts** — all new fields are additive extensions to the existing billing-health response shape. New files: `apps/web/src/app/api/admin/sync-user-by-email/route.ts`, `apps/web/src/app/api/admin/plus-users/route.ts`. Modified: `apps/api/lib/inkwell/square.ex` (with_retry helper), `apps/api/lib/inkwell/billing.ex` (reconcile_all_users rewrite + sync_user_by_email + plus_users_by_source + webhook_stats extension), `apps/api/lib/inkwell_web/controllers/admin_controller.ex` (2 new actions + render_plus_users helper), `apps/api/lib/inkwell_web/router.ex` (2 new admin routes), `apps/web/src/app/admin/billing-health-panel.tsx` (major additions). **Follow-up for admin** (not a code task): verify Square dashboard webhook URL matches `https://api.inkwell.social/api/billing/webhook` and signature key matches `SQUARE_WEBHOOK_SIGNATURE_KEY`; decide policy for the 4 legacy Stripe Plus users (email them, grace period, or auto-downgrade).
- **2026-04-08** — Billing Health Dashboard + Webhook Audit Log + Reconcile-All. Followup to the Square webhook incident: added proactive visibility so a silent webhook failure can never happen invisibly again. **New `webhook_deliveries` table** logs every hit to the webhook endpoint — signature valid or not, parse succeeded or not — capturing `source`, `event_type`, `status` (received / signature_failed / parse_failed / handler_failed / missing_body), `signature_valid`, `remote_ip` (X-Forwarded-For aware), `body_size`, and `error` text. This is the source of truth for "did anything arrive?" (different from `webhook_events` which is the dedup table for successfully processed events). 30-day retention via existing cleanup pattern. **Billing controller logs every webhook hit** to this table before accepting/rejecting, with `Logger.info` for received events including type + source IP. **`Inkwell.Billing.webhook_stats/0`** aggregates: last_delivery_at, totals (24h/7d), status breakdown (24h), active Square subscribers, active Square donors, legacy Stripe users. **`Inkwell.Billing.reconcile_all_users/1`** iterates all non-blocked users with an email (max 1000, most-recent first), calls `sync_from_square/1` on each, and returns a summary: total_checked, plus/donor activated, plus/donor canceled, errors (with up to 20 error details). Safe to run anytime — each sync is idempotent. **2 new admin endpoints**: `GET /api/admin/billing-health` (stats + 20 most recent deliveries) and `POST /api/admin/reconcile-subscriptions` (runs full reconciliation inline, admin-gated). **New `BillingHealthPanel` client component** on the admin dashboard: 6 key metrics (last webhook, events 24h/7d, Square subs, Square donors, legacy Stripe), status breakdown pills, "⚠ No webhooks in 24h but there are active subscribers" warning banner (silent death detector), "Reconcile now" button with confirmation + result summary + error details disclosure, refresh button, and the 20 most recent deliveries with per-row status pill (color-coded: received/processed=green, signature_failed=red, parse_failed/handler_failed/missing_body=amber), event type, remote IP, and relative timestamp. **UTC parsing fix**: Phoenix's `NaiveDateTime` serializes without a timezone suffix (`2026-04-08T19:41:38`), and JS parses that as local time producing negative `timeAgo` values. Added `parseUtc/1` helper that detects missing offset and appends `Z`. **New files**: `20260408000076_create_webhook_deliveries.exs`, `webhook_delivery.ex` schema, `billing-health-panel.tsx`, `api/admin/billing-health/route.ts`, `api/admin/reconcile-subscriptions/route.ts`. **Modified**: `billing.ex` (log_delivery + recent_webhook_deliveries + webhook_stats + reconcile_all_users + cleanup_old_webhook_deliveries), `billing_controller.ex` (log every webhook hit with remote_ip + body_size, retain existing accept/reject behavior), `admin_controller.ex` (billing_health + reconcile_subscriptions actions + render_webhook_delivery helper), `router.ex` (2 new admin routes), `admin/page.tsx` (render BillingHealthPanel below stat cards). No Slack integration per user request — visibility only.
- **2026-04-08** — Self-Healing Square Subscription Sync. User @zaexpcake reported paying for Plus via Square but seeing "free tier" on the billing page. Investigation revealed **zero successful Square webhook events since the migration** — `webhook_events` table empty, `oban_jobs` table has no `WebhookProcessingWorker` entries, and zero users have `square_subscription_id` set. Either the Square dashboard webhook subscription is missing, pointing to the wrong URL, or using a different signature key than `SQUARE_WEBHOOK_SIGNATURE_KEY`. This affected every post-migration subscriber invisibly. **Fix (self-healing sync)**: new `POST /api/billing/sync` endpoint reconciles local user state from Square API on demand as a webhook fallback. `Inkwell.Billing.sync_from_square/1` searches Square customers by email, finds their subscriptions, matches `plan_variation_id` to determine Plus vs Donor, and updates the local user record — clearing any legacy `stripe_subscription_id` at the same time. Idempotent. Returns change list (`[:plus_activated, :donor_activated, :plus_canceled, :donor_canceled]`). **Auto-sync on billing page load**: when user returns from Square checkout (`?checkout=success`) OR page detects `needs_resubscribe: true`, the frontend automatically calls sync then refetches status. Visible "Checking with Square…" banner during explicit syncs; silent for background reconciliation. **Manual button**: "Already paid? Sync from Square" link inside the resubscribe banner. **Square helpers**: new `Square.search_customers_by_email/1` (POST `/customers/search`) and `search_subscriptions_by_customer/1` (POST `/subscriptions/search`, scoped to location ID). **Slack notifications** fire on first-time activation via sync (same as webhook path). No migration needed. New files: `apps/web/src/app/api/billing/sync/route.ts`. Modified: `apps/api/lib/inkwell/square.ex` (2 new search functions), `apps/api/lib/inkwell/billing.ex` (sync_from_square + reconcile_plus + reconcile_donor + pick_newest_active), `apps/api/lib/inkwell_web/controllers/billing_controller.ex` (sync/2 action), `apps/api/lib/inkwell_web/router.ex` (POST /api/billing/sync), `apps/web/src/app/settings/billing/page.tsx` (auto-sync on load, manual sync button, sync status banners). **User action still required**: verify the Square dashboard webhook subscription exists and points to `https://api.inkwell.social/api/billing/webhook` with the correct signature key matching `SQUARE_WEBHOOK_SIGNATURE_KEY`.
