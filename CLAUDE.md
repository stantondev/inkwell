...
- **Database**: 3 tables — `polls` (question, type, status, closes_at, total_votes, creator_id, entry_id), `poll_options` (label, position, vote_count), `poll_votes` (user_id, poll_id, poll_option_id; unique on user+poll)
- **Backend**:
  ...
  - `apps/api/lib/inkwell/polls.ex` — context: CRUD, voting (atomic counter increments), batch queries, `is_poll_open?/1`
  - `apps/api/lib/inkwell_web/controllers/poll_controller.ex` — 10 actions (index, active_widget, show, vote, create_entry_poll, update_entry_poll, admin_index, create_platform, close, delete)
  - Routes: `GET /api/polls`, `GET /api/polls/active`, `GET /api/polls/:id`, `POST /api/polls/:id/vote`, `PATCH /api/polls/:id`, `POST /api/entries/:entry_id/poll` + admin routes
  - Entry controller `show/2` and `show_own/2` include poll data when entry has an attached poll
  - Migration: `20260301000043`
- **Frontend**:
  - `apps/web/src/components/poll-widget.tsx` — shared client component (3 states: voting, results, closed) with optimistic UI
  - `apps/web/src/app/polls/page.tsx` — Community Voice listing
  - `apps/web/src/app/polls/[id]/page.tsx` — single poll detail
  - `apps/web/src/app/admin/polls/page.tsx` — admin poll management
  - 8 proxy routes under `apps/web/src/app/api/`
...
- Latest migration: `20260301000043_create_polls.exs`
