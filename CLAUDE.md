...
### Recently Completed
- **2026-03-01** — Avatar Builder (DiceBear Avataaars). Customizable avatar builder for users who don't want to use a real photo — similar to Reddit's Snoo builder. Uses DiceBear v9 Avataaars style as placeholder (architecture supports swapping to custom art style later). Pre-render architecture: DiceBear JS generates SVG client-side (live preview) → on save, canvas renders to 400x400 JPEG data URI → stored via existing avatar upload endpoint. Dual storage: `avatar_config` (JSONB for re-editing) + `avatar_url` (rendered JPEG for serving/federation). All existing avatar serving, federation, RSS, and display work unmodified. 10 customization categories: Skin Tone, Hair Style, Hair Color, Eyes, Eyebrows, Mouth, Facial Hair, Clothing, Clothing Color, Accessories. Two-panel layout (live preview + scrollable category tabs with option pills/swatches). Randomize button, Save, Revert to photo. Compact mode for onboarding. Backend: `avatar_config :map` (JSONB) field on users, `validate_avatar_config/1` ensures `%{"style" => string, "options" => map}` shape, added to `free_fields` and both `render_user/1` functions. Frontend: `avatar-builder-config.ts` (option definitions, defaults, types), `avatar-render.ts` (SVG-to-JPEG canvas pipeline), `avatar-builder.tsx` (core component with DiceBear code-split via `next/dynamic` + `ssr: false`), Settings → Avatar tab (`settings/avatar/page.tsx` server + `avatar-builder-page.tsx` client wrapper), onboarding Step 1 upload/build segmented toggle. DiceBear optional categories (facialHair, accessories) use `__none` sentinel mapped to probability=0. NPM packages: `@dicebear/core`, `@dicebear/avataaars`. Migration `20260301000042`. New files: `avatar-builder-config.ts`, `avatar-render.ts`, `avatar-builder.tsx`, `settings/avatar/page.tsx`, `avatar-builder-page.tsx`, `20260301000042_add_avatar_config_to_users.exs`. Modified: `user.ex`, `user_controller.ex`, `auth_controller.ex`, `session.ts`, `settings/layout.tsx`, `welcome/page.tsx`, `globals.css`.
