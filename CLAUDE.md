...
- **Scoped to high-signal types only**: `comment`, `reply`, `mention` (mirrors push notification pattern)
...
- **Entry info resolution**: `resolve_entry_info/1` looks up entry by `target_id`, builds title + URL for the email. Falls back to notification data for comment/reply targets
- **Backend**:
