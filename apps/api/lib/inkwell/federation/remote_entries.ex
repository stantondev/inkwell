  ...
  def upsert_remote_entry(attrs) do
    if points_to_self?(attrs) do
      ...
      case Repo.get_by(RemoteEntry, ap_id: attrs.ap_id) do
        ...
        existing ->
          existing
          |> RemoteEntry.changeset(keep_higher_counts(attrs, existing))
          |> Repo.update()
      end
    end
  end

  # A post seen again (an edit, the hashtag poll) must not lower its counts:
  # other servers only know part of a post's replies/boosts/favourites. Only
  # the home server's own numbers (`Engagement.apply_counts/2`) can go down.
  @count_fields [:reply_count, :likes_count, :boosts_count]
  defp keep_higher_counts(attrs, existing) do
    Enum.reduce(@count_fields, attrs, fn field, acc ->
      current = Map.get(existing, field) || 0

      case Map.get(acc, field) do
        n when is_integer(n) and n >= current -> acc
