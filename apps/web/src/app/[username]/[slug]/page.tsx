...
// Fediverse players (PeerTube, Funkwhale, Castopod, Owncast) share a globe.
const FEDIVERSE_ICON = (
...
const SERVICE_ICONS: Record<MusicService, React.ReactNode> = {
  peertube: FEDIVERSE_ICON,
  funkwhale: FEDIVERSE_ICON,
  castopod: FEDIVERSE_ICON,
  owncast: FEDIVERSE_ICON,
  spotify: (
...
function EntryMusicEmbed({ embed, music }: { embed: MusicEmbed; music: string }) {
  ...
  // Fediverse players come from independent servers, so they're sandboxed;
  // video sizes to 16:9.
  if (embed.fediverse) {
    return (
      <div className="music-embed-container mt-6">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <a href={music} target="_blank" rel="noopener noreferrer" className="text-xs ml-auto hover:underline shrink-0" style={{ color: "var(--muted)" }}>
            Open ↗
          </a>
        </div>
        <div className="rounded-xl overflow-hidden shadow-sm">
          <iframe
            src={embed.embedUrl}
            width="100%"
            height={embed.aspect === "video" ? undefined : embed.height}
            frameBorder="0"
            ...
            className="block"
            style={{ border: "none", ...(embed.aspect === "video" ? { aspectRatio: "16 / 9", height: "auto" } : {}) }}
          />
        </div>
      </div>
    );
    ...
    >
      ...
      <div className={moodHue !== null ? "entry-ambient" : ""}>
        ...
        <div className="entry-wide px-4 sm:px-6 md:px-8 lg:px-12 pt-10 pb-12 relative">
          ...
          {/* Stamps — horizontal row inline on mobile, absolute top-right on desktop */}
      ...
      {entry.cover_image_id && (
        <ContentWarning isSensitive={!!entry.is_sensitive} contentWarning={entry.content_warning}>
          <div className="entry-wide px-4 sm:px-6 md:px-8 lg:px-12">
            <div className="w-full overflow-hidden rounded-lg" style={{ maxHeight: "min(420px, 55vw)" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
