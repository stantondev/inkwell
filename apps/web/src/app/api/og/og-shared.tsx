...
export function PenNibIcon({ size = 48, color = "#2d4a8a" }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="1085 -5 415 415" fill="none">
      <path d={PEN_NIB_PATH_1} fill={color} />
      <path d={PEN_NIB_PATH_2} fill={color} />
    </svg>
  );
}
...
/** Lora from public/fonts — dev runs from apps/web, the Docker standalone from the repo root. */
export function loadFonts() {
  if (!fontCache) {
    fontCache = (async () => {
      const cwd = process.cwd();
      const candidates = [join(cwd, "public", "fonts"), join(cwd, "apps", "web", "public", "fonts")];
      for (const dir of candidates) {
        try {
          const [regular, bold] = await Promise.all([
            readFile(join(dir, "Lora-Regular.ttf")),
            readFile(join(dir, "Lora-Bold.ttf")),
          ]);
          return { regular, bold };
        } catch {
          continue;
      ...
      throw new Error("Lora fonts not found");
    })();
    fontCache.catch(() => {
      fontCache = null;
    });
