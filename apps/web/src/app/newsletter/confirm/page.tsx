...
export default function ConfirmPage() {
  ...
  const [writerUsername, setWriterUsername] = useState<string | null>(null);
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          setWriterUsername(data.data?.writer_username ?? null);
          setStatus("success");
        } else {
          setStatus("error");
        }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ background: "var(--background)" }}>
      <div className="w-full max-w-md text-center">
        <div className="rounded-2xl border p-8" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
          {status === "loading" && (
            <p className="text-sm" style={{ color: "var(--muted)" }}>Confirming your subscription...</p>
          )}
          ...
          {status === "success" && (
            <>
              ...
              <p className="text-sm mb-4" style={{ color: "var(--muted)" }}>
                You&apos;ll receive new entries by email whenever they&apos;re published.
              </p>
              {writerUsername && (
                <Link
                  href={`/${writerUsername}`}
          ...
          {status === "error" && (
            <>
              ...
              <p className="text-sm mb-4" style={{ color: "var(--muted)" }}>
                This confirmation link may have expired or already been used. Try subscribing again.
              </p>
              <Link
                href="/"
