...
} from "@/components/live-nav-counts";
...
export function NotificationList({
  ...
  // New notifications arrive while the page is open: the shared nav poll
  // fetches them once and broadcasts them. Prepend any we don't have yet, and
  // pick up state changes (e.g. read elsewhere) on ones we do.
  ...
  useEffect(() => {
    const onArrived = (e: Event) => {
      ...
      if (!detail?.latest?.length) return;
      const known = new Set(notificationsRef.current.map((n) => n.id));
      const added = detail.latest.filter((n) => !known.has(n.id));
      if (added.length === 0) return;
      const addedIds = new Set(added.map((n) => n.id));
      setNotifications((prev) => [...added, ...prev.filter((n) => !addedIds.has(n.id))]);
      setFreshIds((ids) => new Set([...ids, ...addedIds]));
      setServerUnread((v) => v + added.filter((n) => !n.read).length);
    };
    window.addEventListener(NOTIFICATIONS_ARRIVED_EVENT, onArrived);
    return () => window.removeEventListener(NOTIFICATIONS_ARRIVED_EVENT, onArrived);
  }, []);
                ...
                {group.items.map((n, i) => {
                  ...
                  return (
                    <div
                      ...
                      } ${isClickable && !isPendingFollowRequest ? "cursor-pointer hover:brightness-95" : ""}${freshIds.has(n.id) ? " notification-row-fresh" : ""}`}
                      style={{
                        borderColor: "var(--border)",
