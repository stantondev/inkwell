...
export function NotificationSettings() {
  ...
  const toggleSetting = async (
    ...
    try {
      ...
      // the toggle showing the value that was never stored.
      if (!res.ok) {
        setter(!value);
        setSaveError(true);
      }
    ...
    return (
      ...
      >
        ...
        <p className="text-sm" style={{ color: "var(--danger, #dc2626)" }}>
          Couldn&apos;t load your notification settings. Reload the page to try again. Nothing has been changed.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      {saveError && (
        <p
          className="text-sm mt-6 rounded-xl border p-4"
          style={{
