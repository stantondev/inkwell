...
export function CircleEditor({
  ...
  // Mention state — positions are viewport-fixed (for portal rendering at document.body)
  ...
  const editor = useEditor({
    ...
    onBlur: () => {
      // Delay to allow mention dropdown clicks to register
      setTimeout(() => {
        ...
        if (i === 0 || /\s/.test(text[i - 1])) {
          ...
          if (/^[a-zA-Z0-9_-]*$/.test(query) && query.length >= 1) {
            ...
            try {
              ...
              // Use viewport-fixed coordinates for portal rendering
              setMentionPos({
                top: coords.top,
                left: coords.left,
              });
  ...
  // Render the toolbar (shared between compact and full modes)
  const renderToolbar = () => (
    <div className={`circle-editor-toolbar ${compact ? "circle-editor-toolbar-compact" : ""}`}>
  ...
  // Render mention dropdown via portal to escape overflow:hidden containers
  const renderMentionDropdown = () => {
    if (mentionQuery === null || mentionUsers.length === 0 || !mentionPos) return null;
    ...
    return createPortal(
      <div
        style={{
          ...
          left: Math.min(mentionPos.left, window.innerWidth - 280),
  ...
  return (
    <div ref={editorWrapRef} className={`circle-editor-wrap ${compact ? "circle-editor-compact" : ""}`}>
      {/* Toolbar — always shown (compact gets smaller styling) */}
      {renderToolbar()}

      {/* Editor content */}
      <EditorContent editor={editor} />

      {/* Mention dropdown — rendered via portal to escape overflow:hidden */}
      {renderMentionDropdown()}

      {/* Footer — compact mode shows submit button */}
      {compact && onSubmit && (
        <div style={{ display: "flex", justifyContent: "flex-end", padding: "0.375rem 0.5rem", borderTop: "1px solid var(--border)" }}>
          <button
            type="button"
